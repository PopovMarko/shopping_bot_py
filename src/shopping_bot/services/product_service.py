...
from shopping_bot.core.domain import ProductDomain, ProductInputResult
from shopping_bot.core.interfaces import (
    ProductRepositoryInterface,
)
from shopping_bot.core.repository_dto import ProductRecord
...
class ProductController:
    ...
    async def process_product(
        ...
        if confidence >= 85:
            ...
            return to_response_product_domain(ProductInputResult.PRODUCT_FOUND, product)
        elif confidence >= 75:
            product = product_record_list[extraction[2]]
            return to_response_product_domain(
                ProductInputResult.PRODUCT_NOT_FOUND_NEEDS_CONFIRMATION, product
            )
        else:
            product = await self.repository.create_product(product_name)
            return to_response_product_domain(
                ProductInputResult.PRODUCT_CREATED, product
            )
    ...
    async def process_confirmation(
        ...
        if confirmed:
            ...
            return to_response_product_domain(ProductInputResult.PRODUCT_FOUND, product)
        product = await self.repository.create_product(product_name)
        return to_response_product_domain(ProductInputResult.PRODUCT_CREATED, product)


# TODO to think to add quantity field
def to_response_product_domain(
